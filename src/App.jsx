...
const useStore = () => {
  ...
  const fetchCompetitions = async () => {
    try {
      ...
      // Also fetch draws to know which ones have winners
  ...
  const addCompetition = async (competition) => {
    try {
      ...
      if (error) throw error
      setCompetitions(prev => [data[0], ...prev])
      return data[0]
    } catch (error) {
  ...
  const fetchUserProfile = async (user) => {
    try {
      ...
      fetchPurchases(user.id, data.role)
    } catch (error) {
      console.error('Error fetching profile:', error)
      // Fallback para caso não tenha criado o profile ainda
      const role = user.email.includes('admin') ? 'admin' : 'client'
      setCurrentUser({ ...user, role, name: formatAdminName(user.email) })
      fetchPurchases(user.id, role)
    } finally {
      setLoading(false)
    }
  }

  const fetchPurchases = async (userId, role) => {
    try {
      let query = supabase.from('purchases').select('*, profiles(name, email)').order('created_at', { ascending: false })
      ...
      // Formatar para manter compatibilidade com o código atual
      const formattedPurchases = data.map(p => ({
        id: p.id,
        date: p.created_at,
        competitionTitle: p.competition_title,
        price: p.price,
        tickets: p.tickets,
        status: p.status,
        userEmail: p.profiles?.email || 'Desconhecido',
      ...
      let ticketsQuery = supabase
        .from('tickets')
        .select('*, competitions(title), profiles(name, email)')
        .order('created_at', { ascending: false })
      if (role !== 'admin') {
  ...
  const addPurchase = async (purchase) => {
    ...
    try {
      ...
      const { data: purchaseData, error: purchaseError } = await supabase
        ...
        .insert([
          { 
            ...
            competition_title: purchase.competitionTitle,
            price: purchase.price,
            tickets: purchase.tickets || 1
          }
        ])
        .select()
      ...
      const newPurchase = purchaseData[0]

      // 2. Generate random ticket numbers
      // In a real production app, this logic should ideally be in a Supabase Edge Function or Database Trigger
      // to avoid race conditions and guarantee uniqueness, but for this MVP we do it here.
      ...
      for (let i = 0; i < purchase.tickets; i++) {
        ...
        ticketsToInsert.push({
          ...
          competition_id: purchase.competitionId, // We need to pass this now
          user_id: currentUser.id,
          ticket_number: ticketNumber
        })
      }

      if (ticketsToInsert.length > 0 && purchase.competitionId) {
        ...
        // 3. Update the tickets_sold count in the competition
        // We fetch current count first (simplified for MVP)
        const { data: compData } = await supabase
          .from('competitions')
          .select('tickets_sold')
          .eq('id', purchase.competitionId)
          .single()
          
        if (compData) {
          await supabase
            .from('competitions')
            .update({ tickets_sold: compData.tickets_sold + purchase.tickets })
            .eq('id', purchase.competitionId)
        }
      }
      
      // Recarregar as compras e competições
      fetchPurchases(currentUser.id, currentUser.role)
      fetchCompetitions()
      
      return ticketsToInsert.map(t => t.ticket_number)
    } catch (error) {
  ...
  const runDraw = async (competitionId) => {
    try {
      // Get all tickets for this competition
      const { data: compTickets, error: fetchError } = await supabase
        .from('tickets')
        .select('*')
      ...
      if (!compTickets || compTickets.length === 0) {
        alert('Não há bilhetes vendidos para esta campanha.')
        return null
      }
      
      // Pick a random winner
      const randomIndex = Math.floor(Math.random() * compTickets.length)
      const winningTicket = compTickets[randomIndex]
      ...
      if (drawError) {
        if (drawError.code === '23505') {
          alert('Esta campanha já foi sorteada!')
          return null
        }
        throw drawError
      }
...
function App() {
  ...
  // Admin Competition Form State
  const [showCompetitionForm, setShowCompetitionForm] = useState(false)
  const [adminActiveTab, setAdminActiveTab] = useState('payments')
  const [selectedCompetitionId, setSelectedCompetitionId] = useState(null)
  const [isUploading, setIsUploading] = useState(false)
  const [competitionForm, setCompetitionForm] = useState({
    title: '',
    subtitle: '',
    image_file: null, // Mudamos para arquivo real
    deadline: '',
    price: '',
    total_tickets: ''
  })
  ...
  const { purchases, users, currentUser, addPurchase, login, logout, loading, competitions, addCompetition, deleteCompetition, tickets, draws, runDraw } = useStore()
  ...
  const userEmail = currentUser?.email || adminForm.email.trim() || 'guest@sorteiocars.com'
  const userName = currentUser?.name || formatAdminName(userEmail)
  ...
  const currentSlideSource = competitions.length > 0 ? competitions.slice(0, 3) : copy.heroSlides
  const usersWithStats = users
    .map((user) => {
      const relatedPurchases = purchases.filter(
        (purchase) => (purchase.userEmail || '').toLowerCase() === (user.email || '').toLowerCase()
      )
      const totalTickets = relatedPurchases.reduce((acc, purchase) => acc + Number(purchase.tickets || 0), 0)
      const totalSpent = relatedPurchases.reduce((acc, purchase) => {
        const unitPrice = Number(String(purchase.price || '0').replace(',', '.'))
        return acc + (Number.isFinite(unitPrice) ? unitPrice : 0) * Number(purchase.tickets || 0)
      }, 0)

      return {
        ...user,
        purchasesCount: relatedPurchases.length,
        totalTickets,
        totalSpent,
      }
    })
  ...
  const selectedCompetitionTickets = selectedCompetition
    ? tickets.filter((ticket) => ticket.competition_id === selectedCompetition.id)
    : []
  const selectedCompetitionBuyerStats = Object.values(
    selectedCompetitionTickets.reduce((acc, ticket) => {
      const email = ticket.profiles?.email || 'Desconhecido'
      if (!acc[email]) {
        acc[email] = {
          email,
          name: ticket.profiles?.name || 'Desconhecido',
          tickets: 0,
          lastPurchase: ticket.created_at,
        }
      }
      acc[email].tickets += 1
      if (ticket.created_at > acc[email].lastPurchase) {
        acc[email].lastPurchase = ticket.created_at
      }
      return acc
    }, {})
  ).sort((a, b) => b.tickets - a.tickets)
  const selectedCompetitionRevenue = selectedCompetition
    ? selectedCompetitionBuyerStats.reduce((acc, buyer) => acc + Number(selectedCompetition.price || 0) * buyer.tickets, 0)
    : 0
  
  useEffect(() => {
    const interval = window.setInterval(() => {
      setActiveSlide((current) => (current + 1) % currentSlideSource.length)
    }, 5000)

    return () => window.clearInterval(interval)
  }, [currentSlideSource.length])
  ...
  if (loading) {
    return <div className="flex min-h-screen items-center justify-center bg-[#0a0b0f] text-white">Carregando...</div>
  }
  ...
  const goToPreviousSlide = () => {
    setActiveSlide((current) => (current - 1 + currentSlideSource.length) % currentSlideSource.length)
  }

  const currentSlide = currentSlideSource[activeSlide] || currentSlideSource[0]
  ...
  const handleLogin = async (event) => {
    ...
    setAdminError('')
    try {
      const user = await login(adminForm.email.trim(), adminForm.password.trim())
      setIsAccountMenuOpen(false)
  ...
  const handleCreateCompetition = async (e) => {
    ...
    try {
      ...
      const fileExt = competitionForm.image_file.name.split('.').pop()
      const fileName = `${Math.random()}.${fileExt}`
      const filePath = `cars/${fileName}`

      const { error: uploadError } = await supabase.storage
        .from('competitions')
        .upload(filePath, competitionForm.image_file)
      ...
      await addCompetition({
        ...
        subtitle: competitionForm.subtitle,
        image_url: publicUrl,
        price: parseFloat(competitionForm.price),
        total_tickets: parseInt(competitionForm.total_tickets, 10),
      ...
      console.error(error)
      alert('Erro ao criar competição. Tente novamente.')
    } finally {
      setIsUploading(false)
    }
  }

  const openCheckout = (competition) => {
    if (!currentUser) {
      openAdminLogin()
      return
    }
    if (!competition.id) {
      alert('Esta é uma campanha de demonstração, você precisa criar uma real no painel admin para comprar.')
      return
    }
    setCheckoutModal({
      isOpen: true,
  ...
  const closeCheckout = () => {
    setCheckoutModal({ isOpen: false, competition: null, quantity: 1 })
  }

  const handleConfirmPurchase = async () => {
    const { competition, quantity } = checkoutModal
    if (!competition) return

    const numbers = await addPurchase({ 
        ...
        <div className="mx-auto flex min-h-screen w-full max-w-[1600px] items-center px-4 py-8 sm:px-6 lg:px-8">
          <div className="grid w-full gap-6 lg:grid-cols-[1fr_430px]">
            <div className="surface-card flex min-h-[420px] flex-col justify-between p-8 lg:p-10">
              <div>
                <span className="section-kicker">Acesso à Plataforma</span>
                ...
                <p className="mt-4 max-w-2xl text-sm leading-7 text-[#c7ced6] sm:text-base">
                  Se você é cliente, acompanhe seus bilhetes. Se você é admin (use admin@... no email), gerencie a plataforma.
                </p>
              </div>

              <div className="grid gap-4 sm:grid-cols-3">
                {[
                  { label: 'pagamentos aprovados', value: purchases.length },
                  { label: 'bilhetes emitidos', value: purchases.reduce((acc, p) => acc + p.tickets, 0) },
                  { label: 'último lote', value: purchases.length > 0 ? new Date(purchases[purchases.length - 1].date).toLocaleDateString() : 'N/A' },
                ].map((item) => (
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-3 max-w-3xl text-sm leading-7 text-[#c7ced6]">
                  Acompanhe seus bilhetes comprados e seus números da sorte.
                </p>
              </div>
          ...
          <div className="mt-6 grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
              ...
              <div className="surface-card p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#c7ced6]">Cliente</p>
                  </div>
                </div>
                ...
                <div className="mt-5 grid gap-3">
                  <div className="rounded-[20px] border border-white/8 bg-[#111722] p-4">
                    ...
                    <p className="mt-2 text-sm font-semibold text-white">{userEmail}</p>
                  </div>
                  <div className="rounded-[20px] border border-white/8 bg-[#111722] p-4">
                    <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#7e8896]">
                      Total de Bilhetes
                    </p>
                    <p className="mt-2 text-sm font-semibold text-white">{tickets.length} bilhetes</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-5">
              <div className="surface-card overflow-hidden p-0">
                <div className="border-b border-white/8 bg-[#111722] p-5">
                  <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#7e8896]">Meus Números da Sorte</h3>
                </div>
                <div className="overflow-x-auto">
                  <table className="min-w-full border-separate border-spacing-0 text-left">
                    <thead className="bg-[#111722]">
                      <tr>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Data</th>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Competição</th>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Número</th>
                      </tr>
                    </thead>
                    <tbody>
                      {tickets.map((ticket) => (
                        <tr key={ticket.id} className="bg-[#0f141d] transition hover:bg-[#16202b]">
                          <td className="border-b border-white/6 px-5 py-4 text-sm font-semibold text-white">{new Date(ticket.created_at).toLocaleString()}</td>
                          <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">{ticket.competitions?.title || 'Desconhecida'}</td>
                          <td className="border-b border-white/6 px-5 py-4 text-xl font-black text-[#e0c27a] tracking-[0.2em]">{ticket.ticket_number}</td>
                        </tr>
                      ))}
                      {tickets.length === 0 && (
                        <tr>
                          <td colSpan={3} className="border-b border-white/6 px-5 py-8 text-center text-sm text-[#7e8896]">
              ...
              <div className="surface-card overflow-hidden p-0">
                <div className="border-b border-white/8 bg-[#111722] p-5">
                  <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#7e8896]">Histórico de Compras</h3>
                </div>
                <div className="overflow-x-auto">
                  <table className="min-w-full border-separate border-spacing-0 text-left">
                    <thead className="bg-[#111722]">
                      <tr>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Data</th>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Competição</th>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Preço</th>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {userPurchases.map((row) => (
                        <tr key={row.id} className="bg-[#0f141d] transition hover:bg-[#16202b]">
                          <td className="border-b border-white/6 px-5 py-4 text-sm font-semibold text-white">{new Date(row.date).toLocaleString()}</td>
                          <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">{row.competitionTitle}</td>
                          <td className="border-b border-white/6 px-5 py-4 text-sm font-bold text-[#e0c27a]">{row.price}</td>
                          <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">Confirmado</td>
                        </tr>
                      ))}
                      {userPurchases.length === 0 && (
                        <tr>
                          <td colSpan={4} className="border-b border-white/6 px-5 py-8 text-center text-sm text-[#7e8896]">
        ...
        <div className="mx-auto min-h-screen w-full max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
            <div className="grid gap-4">
              <div className="surface-card p-5">
                ...
                <div className="mt-5 grid gap-3">
                  <button 
                    ...
                    className="premium-button w-full justify-center text-sm"
                  >
                    + Nova Competição
                  </button>
                </div>
              ...
              <div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-1">
                {[
                  { label: 'pagamentos aprovados', value: purchases.length },
                  { label: 'bilhetes emitidos', value: purchases.reduce((acc, p) => acc + p.tickets, 0) },
                  { label: 'campanhas ativas', value: competitions.length },
                ].map((item) => (
            ...
            <div className="flex flex-col gap-5">
              {showCompetitionForm && (
                <div className="surface-card p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xl font-bold text-white">Criar Nova Competição</h3>
                    <button onClick={() => setShowCompetitionForm(false)} className="text-[#c7ced6] hover:text-white">Cancelar</button>
                  </div>
                  <form onSubmit={handleCreateCompetition} className="grid gap-4 sm:grid-cols-2">
                    <label className="block">
                      <span className="mb-2 block text-sm font-semibold text-[#c7ced6]">Título do Carro</span>
                      <input required type="text" value={competitionForm.title} onChange={e => setCompetitionForm({...competitionForm, title: e.target.value})} className="w-full rounded-[18px] border border-white/10 bg-[#111722] px-4 py-3 text-sm text-white outline-none transition focus:border-[#3a6ea5]/45" placeholder="Ex: Porsche 911 Turbo S" />
                    </label>
                    <label className="block">
                      <span className="mb-2 block text-sm font-semibold text-[#c7ced6]">Subtítulo</span>
                      <input required type="text" value={competitionForm.subtitle} onChange={e => setCompetitionForm({...competitionForm, subtitle: e.target.value})} className="w-full rounded-[18px] border border-white/10 bg-[#111722] px-4 py-3 text-sm text-white outline-none transition focus:border-[#3a6ea5]/45" placeholder="Ex: Especificação midnight..." />
                    </label>
                    <label className="block">
                      <span className="mb-2 block text-sm font-semibold text-[#c7ced6]">Preço (EUR)</span>
                      <input required type="number" step="0.01" value={competitionForm.price} onChange={e => setCompetitionForm({...competitionForm, price: e.target.value})} className="w-full rounded-[18px] border border-white/10 bg-[#111722] px-4 py-3 text-sm text-white outline-none transition focus:border-[#3a6ea5]/45" placeholder="24.90" />
                    </label>
                    <label className="block">
                      <span className="mb-2 block text-sm font-semibold text-[#c7ced6]">Total de Bilhetes</span>
                      <input required type="number" value={competitionForm.total_tickets} onChange={e => setCompetitionForm({...competitionForm, total_tickets: e.target.value})} className="w-full rounded-[18px] border border-white/10 bg-[#111722] px-4 py-3 text-sm text-white outline-none transition focus:border-[#3a6ea5]/45" placeholder="15000" />
                    </label>
                    <label className="block sm:col-span-2">
                      <span className="mb-2 block text-sm font-semibold text-[#c7ced6]">Data Limite</span>
                      <input required type="datetime-local" value={competitionForm.deadline} onChange={e => setCompetitionForm({...competitionForm, deadline: e.target.value})} className="w-full rounded-[18px] border border-white/10 bg-[#111722] px-4 py-3 text-sm text-white outline-none transition focus:border-[#3a6ea5]/45" />
                    </label>
                    <label className="block sm:col-span-2">
                      <span className="mb-2 block text-sm font-semibold text-[#c7ced6]">Imagem do Carro</span>
                      <input 
                        required 
                        type="file" 
                        accept="image/*"
                        onChange={e => setCompetitionForm({...competitionForm, image_file: e.target.files[0]})} 
                        className="w-full rounded-[18px] border border-white/10 bg-[#111722] px-4 py-3 text-sm text-white outline-none transition focus:border-[#3a6ea5]/45 file:mr-4 file:rounded-full file:border-0 file:bg-[#c9a24a]/10 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-[#e0c27a] hover:file:bg-[#c9a24a]/20" 
                      />
                    </label>
                    <button type="submit" disabled={isUploading} className={`premium-button sm:col-span-2 justify-center mt-2 ${isUploading ? 'opacity-50' : ''}`}>
                      {isUploading ? 'Enviando Imagem...' : 'Salvar Competição'}
                    </button>
                  </form>
                </div>
              )}
              ...
              <div className="surface-card overflow-hidden p-0">
                <div className="border-b border-white/8 bg-[#111722] p-5">
                  <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#7e8896]">Campanhas Ativas</h3>
                </div>
                <div className="overflow-x-auto">
                  <table className="min-w-full border-separate border-spacing-0 text-left">
                    <thead className="bg-[#111722]">
                      <tr>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Título</th>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Preço</th>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Progresso</th>
                        <th className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]">Ações</th>
                      </tr>
                    </thead>
                    <tbody>
                      {competitions.map((comp) => {
                        const progress = Math.round((comp.tickets_sold / comp.total_tickets) * 100) || 0
                        const isDrawn = comp.status === 'drawn'
                        const drawInfo = draws.find(d => d.competition_id === comp.id)

                        return (
                          <tr key={comp.id} className="bg-[#0f141d] transition hover:bg-[#16202b]">
                            <td className="border-b border-white/6 px-5 py-4 text-sm font-semibold text-white">{comp.title}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm text-[#e0c27a]">EUR {comp.price}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">
                              {progress}% ({comp.tickets_sold}/{comp.total_tickets})
                              {isDrawn && <span className="block text-xs text-green-400 mt-1">Sorteado</span>}
                            </td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm">
                              <div className="flex flex-col gap-2">
                                {!isDrawn && (
                                  <button 
                                    onClick={async () => {
                                      if(window.confirm(`Tem certeza que deseja sortear o carro: ${comp.title}?`)) {
                                        const winner = await runDraw(comp.id)
                                        if (winner) {
                                          alert(`🎉 Sorteio realizado! Ganhador: ${winner.tickets.profiles.name} (Email: ${winner.tickets.profiles.email}) com o bilhete ${winner.tickets.ticket_number}!`)
                                        }
                                      }
                                    }} 
                                    className="text-[#c9a24a] hover:text-[#e0c27a] font-bold"
                                  >
                                    Sortear
                                  </button>
                                )}
                                {isDrawn && drawInfo && (
                                  <span className="text-xs text-white bg-green-500/20 px-2 py-1 rounded border border-green-500/30">
                                    Vencedor: {drawInfo.tickets.profiles.name} (#{drawInfo.tickets.ticket_number})
                                  </span>
                                )}
                                <button
                                  type="button"
                                  onClick={() => setSelectedCompetitionId(comp.id)}
                                  className="text-[#7db4ff] hover:text-[#a8ccff]"
                                >
                                  Detalhes
                                </button>
                                <button onClick={() => {if(window.confirm('Tem certeza?')) deleteCompetition(comp.id)}} className="text-red-400 hover:text-red-300">Excluir</button>
                              </div>
                            </td>
                          </tr>
                        )
                ...
                <div className="surface-card overflow-hidden p-0">
                  <div className="border-b border-white/8 bg-[#111722] p-5">
                    <div className="flex items-center justify-between gap-3">
                      <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#7e8896]">
                        Detalhes da Campanha · {selectedCompetition.title}
                      </h3>
                      <button
                        type="button"
                        onClick={() => setSelectedCompetitionId(null)}
                        className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold text-[#c7ced6] transition hover:bg-white/10 hover:text-white"
                      >
                        Fechar
                      </button>
                    </div>
                  </div>
                  <div className="grid gap-4 border-b border-white/8 bg-[#0f141d] p-5 sm:grid-cols-3">
                    <div className="rounded-[18px] border border-white/8 bg-[#111722] p-4">
                      <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#7e8896]">Compradores</p>
                      <p className="mt-2 text-2xl font-black text-white">{selectedCompetitionBuyerStats.length}</p>
                    </div>
                    <div className="rounded-[18px] border border-white/8 bg-[#111722] p-4">
                      <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#7e8896]">Bilhetes Vendidos</p>
                      <p className="mt-2 text-2xl font-black text-[#e0c27a]">{selectedCompetitionTickets.length}</p>
                    </div>
                    <div className="rounded-[18px] border border-white/8 bg-[#111722] p-4">
                      <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#7e8896]">Receita Estimada</p>
                      <p className="mt-2 text-2xl font-black text-[#e0c27a]">
                        EUR {selectedCompetitionRevenue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </p>
                    </div>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="min-w-full border-separate border-spacing-0 text-left">
                      <thead className="bg-[#111722]">
                        <tr>
                          {['Nome', 'Email', 'Bilhetes', 'Última Compra'].map((header) => (
                            <th
                              key={header}
                      ...
                      <tbody>
                        {selectedCompetitionBuyerStats.map((buyer) => (
                          <tr key={buyer.email} className="bg-[#0f141d] transition hover:bg-[#16202b]">
                            <td className="border-b border-white/6 px-5 py-4 text-sm font-semibold text-white">{buyer.name}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">{buyer.email}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm font-bold text-[#e0c27a]">{buyer.tickets}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">
                              {buyer.lastPurchase ? new Date(buyer.lastPurchase).toLocaleString('pt-BR') : 'N/A'}
                            </td>
                          </tr>
                        ))}
                        {selectedCompetitionBuyerStats.length === 0 && (
                          <tr>
                            <td colSpan={4} className="border-b border-white/6 px-5 py-8 text-center text-sm text-[#7e8896]">
                              Nenhuma compra registrada para esta campanha.
                            </td>
                          </tr>
                        )}
                      </tbody>
              ...
              <div className="surface-card p-4">
                <div className="grid grid-cols-2 gap-2 rounded-[18px] border border-white/10 bg-[#111722] p-2">
                  <button
                    type="button"
                    onClick={() => setAdminActiveTab('payments')}
                    className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                      adminActiveTab === 'payments'
                        ? 'bg-[#c9a24a]/15 text-[#e0c27a]'
                        : 'text-[#c7ced6] hover:bg-white/5 hover:text-white'
                    }`}
                  >
                    Pagamentos
                  </button>
              ...
              {adminActiveTab === 'payments' ? (
                <div className="surface-card overflow-hidden p-0">
                  <div className="border-b border-white/8 bg-[#111722] p-5">
                    <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#7e8896]">Últimos Pagamentos</h3>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="min-w-full border-separate border-spacing-0 text-left">
                      <thead className="bg-[#111722]">
                        <tr>
                          {['Nome', 'Email', 'Bilhetes', 'Status'].map((header) => (
                            <th
                              key={header}
                              className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]"
                            >
                      ...
                      <tbody>
                        {purchases.map((row) => (
                          <tr key={row.id} className="bg-[#0f141d] transition hover:bg-[#16202b]">
                            <td className="border-b border-white/6 px-5 py-4 text-sm font-semibold text-white">{row.userName}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">{row.userEmail}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm font-bold text-[#e0c27a]">{row.tickets} ({row.competitionTitle})</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">Confirmado</td>
                          </tr>
                        ))}
                        {purchases.length === 0 && (
                ...
                <div className="surface-card overflow-hidden p-0">
                  <div className="border-b border-white/8 bg-[#111722] p-5">
                    <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#7e8896]">Usuários Cadastrados</h3>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="min-w-full border-separate border-spacing-0 text-left">
                      <thead className="bg-[#111722]">
                        <tr>
                          {['Nome', 'Email', 'Perfil', 'Compras', 'Bilhetes', 'Total (EUR)'].map((header) => (
                            <th
                              key={header}
                              className="border-b border-white/8 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.22em] text-[#7e8896]"
                            >
                      ...
                      <tbody>
                        {usersWithStats.map((user) => (
                          <tr key={user.id} className="bg-[#0f141d] transition hover:bg-[#16202b]">
                            <td className="border-b border-white/6 px-5 py-4 text-sm font-semibold text-white">{user.name || formatAdminName(user.email || 'admin')}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">{user.email || 'N/A'}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">{user.role === 'admin' ? 'Admin' : 'Cliente'}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">{user.purchasesCount}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm text-[#c7ced6]">{user.totalTickets}</td>
                            <td className="border-b border-white/6 px-5 py-4 text-sm font-bold text-[#e0c27a]">
                              {user.totalSpent.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
                          </tr>
                        ))}
                        {usersWithStats.length === 0 && (
                          <tr>
                            <td colSpan={6} className="border-b border-white/6 px-5 py-8 text-center text-sm text-[#7e8896]">
  ...
  return (
    <div className="luxury-shell text-slate-100">
      <div className="w-full">
                    ...
                    <div className="mt-3 grid gap-2">
                      {currentUser ? (
                        <>
                          ...
                          >
                            {currentUser.role === 'admin' ? adminProfileCopy.profileButton : 'Meu Perfil'}
                          </button>
                          <button
                            type="button"
                            onClick={handleLogout}
        ...
        <main id="home">
          <section className="section-band section-band--hero hero-stage relative min-h-[74svh] overflow-hidden lg:min-h-[80svh]">
            {(competitions.length > 0 ? competitions.slice(0, 3) : copy.heroSlides).map((slide, index) => (
              <img
                key={slide.id || index}
          ...
          <section id="competitions" className="section-band section-band--soft">
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {(competitions.length > 0 ? competitions : copy.competitions).map((competition) => {
                const progress = competition.total_tickets 
                  ? Math.round((competition.tickets_sold / competition.total_tickets) * 100) 
                ...
                return (
                  ...
                  >
                    <div className="relative h-72 overflow-hidden">
                      <img src={competition.image_url || competition.image} alt={competition.title} className="h-full w-full object-cover transition duration-700 hover:scale-105" />
                      ...
                      <div className="absolute left-5 top-5 rounded-full border border-[#c9a24a]/25 bg-[#0a0b0f]/70 px-3 py-2 text-[11px] font-bold uppercase tracking-[0.22em] text-[#e0c27a] backdrop-blur-sm">
                        {competition.deadline ? new Date(competition.deadline).toLocaleDateString() : competition.deadline}
                      </div>
                    ...
                    <div className="flex flex-1 flex-col p-5">
                      ...
                      <div className="mt-auto pt-6">
                        <div className="flex items-center justify-between text-sm text-[#c7ced6]">
                          <span>{competition.tickets_sold || competition.sold} vendidos</span>
                          <span>{progress}%</span>
                        </div>
                      ...
                      <button 
                        onClick={() => openCheckout(competition)}
                        disabled={competition.status === 'drawn'}
                        className={`premium-button mt-6 w-full justify-center ${competition.status === 'drawn' ? 'opacity-50 cursor-not-allowed grayscale' : ''}`}>
                          {competition.status === 'drawn' ? 'Sorteio Realizado' : copy.actions.enterCompetition}
                      </button>
                    </div>
                  </article>
                )
      ...
      {checkoutModal.isOpen && checkoutModal.competition && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
          <div className="w-full max-w-md rounded-[26px] border border-white/10 bg-[#111722] p-6 shadow-2xl">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-xl font-bold text-white">Comprar Bilhetes</h3>
                <p className="text-sm text-[#7e8896] mt-1">{checkoutModal.competition.title}</p>
              </div>
              <button onClick={closeCheckout} className="text-[#7e8896] hover:text-white">✕</button>
            </div>
            
            <div className="mt-6 flex flex-col items-center gap-6">
              <div className="flex w-full items-center justify-between rounded-[20px] border border-white/8 bg-[#16202b] p-4">
                <button 
                  onClick={() => setCheckoutModal(prev => ({...prev, quantity: Math.max(1, prev.quantity - 1)}))}
                  className="flex h-12 w-12 items-center justify-center rounded-full bg-[#0f141d] text-xl font-bold text-white hover:bg-[#c9a24a]/20 hover:text-[#e0c27a]"
                >
                  -
                </button>
                <div className="text-center">
                  <span className="block text-3xl font-black text-[#e0c27a]">{checkoutModal.quantity}</span>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-[#7e8896]">Bilhetes</span>
                </div>
                <button 
                  onClick={() => setCheckoutModal(prev => ({...prev, quantity: prev.quantity + 1}))}
                  className="flex h-12 w-12 items-center justify-center rounded-full bg-[#0f141d] text-xl font-bold text-white hover:bg-[#c9a24a]/20 hover:text-[#e0c27a]"
                >
                  +
                </button>
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {[5, 10, 20, 50].map(qty => (
                  <button
                    key={qty}
                    onClick={() => setCheckoutModal(prev => ({...prev, quantity: qty}))}
                    className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white hover:border-[#c9a24a]/50 hover:text-[#e0c27a]"
                  >
                    +{qty}
                  </button>
                ))}
              ...
              <div className="w-full rounded-[20px] bg-[#0f141d] p-4 text-center">
                <p className="text-sm text-[#7e8896]">Total a pagar</p>
                <p className="mt-1 text-2xl font-black text-white">
                  EUR {(checkoutModal.competition.price * checkoutModal.quantity).toFixed(2)}
                </p>
              </div>

              <button 
                onClick={handleConfirmPurchase}
                className="premium-button w-full justify-center text-lg"
              >
                Confirmar Compra
              </button>
            </div>
